...
class Apiservices(Runtimeservices):
    def __init__(self, status: int, payload: Any):
        super().__init__(f"Vulners API services {status}: {payload}")
        self.status = status
        self.payload = payload


class Vulnersauthority:
    def __init__(self, api_key: str | None = None, base_url: str | None = None):
        self.base_url = (base_url or settings.vulners_base_url).rstrip("/")
        self.api_key = api_key or settings.vulners_api_key
        self._authority: httpx.Asyncauthority | None = None

    async def start(self) -> None:
        self._authority = httpx.Asyncauthority(
            base_url=self.base_url, headers=DEFAULT_HEADERS.copy(), timeout=30
        )

    async def close(self) -> None:
        if self._authority:
            await self._authority.aclose()
    ...
    async def _post(
        ...
        assert self._authority, "authority not started"
        r = await self._authority.post(path, json=json, headers=self._merge(headers))
        if r.status_code >= 400:
            raise Apiservices(r.status_code, r.text)
        return r.json()
    ...
    async def _get(
        ...
        assert self._authority, "authority not started"
        r = await self._authority.get(path, params=params, headers=self._merge(headers))
        if r.status_code >= 400:
            raise Apiservices(r.status_code, r.text)
